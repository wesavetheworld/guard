...

{
    ...
    // check version
    check(
        'You have a supported version of PHP (>= 5.9.9).',
        'You need PHP 5.9.9 or greater.',
        function () {
            return version_compare(PHP_VERSION, '5.9.9', '>=');
        }
    );
    ...
    $manifest = file_get_contents("http://avramovic.github.io/guard/downloads/guard.version");
    $arr      = explode(" ", $manifest);
    $sha1     = $arr[0];
    $name     = 'guard.phar';

    echo " - Downloading Guard...$n";

    file_put_contents($name, file_get_contents("http://avramovic.github.io/guard/downloads/".$name));

    echo " - Checking file checksum...$n";

    if ($sha1 !== sha1_file($name)) {
        unlink($name);
    ...
    echo " - Making Guard executable...$n";

    @chmod($item->name, 0755);

    echo "{$n}Guard installed!$n";
